...
def main():
    ...
    gold_labels["MAFALDA Label"].replace({"ad hominem": "Abusive Ad Hominem", "appeal to (false) authority": "Appeal to False Authority"}, inplace=True)
    ...
    merged_df_labels = pd.concat([gold_labels, valid.drop(columns="MAFALDA Superlabel").dropna(), downsampled.drop(columns="MAFALDA Superlabel").dropna()], ignore_index=True)

    merged_df_superlabels = pd.concat([gold_superlabels, valid.drop(columns="MAFALDA Label").dropna(), downsampled.drop(columns="MAFALDA Label").dropna()], ignore_index=True)

    merged_df_labels["MAFALDA Label"] = merged_df_labels["MAFALDA Label"].apply(lambda x:str(x).lower())
    merged_df_superlabels["MAFALDA Superlabel"] = merged_df_superlabels["MAFALDA Superlabel"].apply(lambda x:str(x).lower())
    ...
    vocab = merged_df_labels.groupby(['MAFALDA Label', "Dataset"])['Input'].apply(lambda x: len(set(" ".join(x).split()))).reset_index()
    ...
    # Vocab size per class
    # This is actually a pointless visualization now that I think about it
    vocab = merged_df_superlabels.groupby(['MAFALDA Superlabel', "Dataset"])['Input'].apply(lambda x: len(set(" ".join(x).split()))).reset_index()
    plt.figure()
    plt.xticks(rotation=90)
    sns.barplot(
        data=vocab,
